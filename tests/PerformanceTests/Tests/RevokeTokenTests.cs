...
public static class RevokeTokenTests
{
    public static ScenarioProps CreateScenario(HttpClient httpClient)
    {
        var scenario = Scenario.Create("revoke_token_scenario", async context =>
        {
            ...
            var request = Http.CreateRequest("POST", $"{ConfigurationHelper.ApiBaseUrl}/api/users/revoke-token")
